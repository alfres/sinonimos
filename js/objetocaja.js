...
pausa = ((480 + b.children.length * 2) - (nivel * 10))
 ...
 box.style.top = (parseInt(blok.style.top) + (altocaja / (nfilas / 2)) - ((altocaja / nfilas) * parseInt(pa.title)))
box.style.left= parseInt(blok.style.left)
...
function izquierda() {
	...
	blok.style.left = (parseInt(blok.style.left) - anchocaja) ; blok.h = blok.h - 1
}
} 
 function derecha() {
 	if ( rejilla[(blok.h+1)][(blok.v)] == false){
...
blok.style.top = (parseInt(blok.style.top) + altop)
blok.v = (blok.v + 1)
